...
exp_name += '_y_only' if y_only else ''

## Load configs
curr_dir = os.path.dirname(os.path.abspath(__name__))
config_path = os.path.join(curr_dir,'tabdiff', 'configs', 'tabdiff_configs.toml')
raw_config = src.load_config(config_path)

print(f"{mode.capitalize()} Mode is Enabled")
...
if mode == 'train':
    ...
    if ckpt_path is None:
        ckpt_parent_path = os.path.join(curr_dir, 'tabdiff', 'ckpt', dataname, exp_name)
        ckpt_path_arr = os.path.join(ckpt_parent_path, 'best_ema_model_2.1671_4588.pt')
...
train_data = TabDiffDataset(dataname, data_dir, info, y_only=y_only, isTrain=True, dequant_dist=raw_config['data']['dequant_dist'], int_dequant_factor=raw_config['data']['int_dequant_factor'])
...
val_data = TabDiffDataset(dataname, data_dir, info, y_only=y_only, isTrain=False, dequant_dist=raw_config['data']['dequant_dist'], int_dequant_factor=raw_config['data']['int_dequant_factor'])
...
if not os.path.exists(val_data_path):
    print(f"{dataname} does not have its validation set. During MLE evaluation, a validation set will be splitted from the training set!")
    val_data_path = None
if mode == 'train':
    metric_list = ["density"]
else:
...
y_only_model = None
# if impute:
#     y_only_model_path = y_only_model_path
...
if not y_only and not non_learnable_schedule:
...
logger = wandb.init(
    ...
    name=exp_name,
    config=raw_config,
    mode='disabled' if debug or no_wandb else 'online',
)
...
if mode == 'test':
    if report:
        if  is_dcr:
            trainer.report_test_dcr(num_runs)
        else:
            trainer.report_test(num_runs)
    # elif impute:
    #     imputed_sample_save_dir = os.path.join("impute", dataname, exp_name)
    #     trainer.test_impute(
    #         trial_start, trial_size, 
    #         resample_rounds, 
    #         impute_condition, 
